...
#'
#' @examples
#' @import dplyr ggplot2 leaflet tidyr purrr gt janitor
graficar_total_votacion <- function(info) {
  names(info$colores) <-
    stringr::str_replace(stringr::str_to_upper(names(info$colores)),
                         pattern = "_",
                         replacement = ": ")
  info$bd %>%
    filter(!!sym(info$unidad_analisis) == info$id_unidad_analisis) %>%
    summarise(across(starts_with(
      paste("votos", info$competidores,
            sep = "_")
    ),
    ~ sum(.x, na.rm = T)),
    nominal = sum(nominal, na.rm = T)) %>%
    tidyr::pivot_longer(
      cols = starts_with("votos"),
      names_to = "partido",
      values_to = "votacion",
      names_prefix = "votos_"
    ) %>%
    mutate(
      pct = votacion / nominal,
      partido = stringr::str_replace(
        stringr::str_to_upper(partido),
        pattern = "_",
        replacement = ": "
      )
    ) %>%
    ...
    ggfittext::geom_bar_text(outside = T,
                             ...
                             aes(
                               label = glue::glue(
                                 "{scales::comma(votacion, accuracy=1)}\n
                                                  ({scales::percent(pct)})"
                               )
                             )) +
    scale_fill_manual(values = info$colores) +
    labs(
      title = "Resultados elección",
      subtitle = info$nombre_unidad_analisis,
      x = "",
      y = "Votos",
...
#'
#' @examples
graficar_distibucion <- function(info = info,
                                 analisis,
                                 comparar = F) {
  if (!comparar)
    info$bd <-
      info$bd %>% filter(!!sym(info$unidad_analisis) == info$id_unidad_analisis)
  color <- info$colores[analisis]
  g <- info$bd %>%
    mutate(across(contains(analisis), ~ .x / nominal)) %>%
    ggplot(aes(
      x = as.factor(!!sym(info$unidad)),
      y = !!sym(glue::glue("votos_{analisis}")),
      alpha = !!sym(info$unidad_analisis) == info$id_unidad_analisis
    )) +
    ...
    labs(
      title = "Distribución del porcentaje de votación respecto a la lista nominal",
...
#'
#' @examples
graficar_correlacion_partidista <-
  function(partidos, info_base, info_contraste) {
    b1 <- info_base$bd %>%
      group_by(seccion) %>%
      summarise(across(
        glue::glue("votos_{info$competidores}"),
    ...
    b2 <- info_contraste$bd %>%
      group_by(seccion) %>%
      summarise(across(
        glue::glue("votos_{info2$competidores}"),
        ~ sum(.x, na.rm = T) / sum(nominal, na.rm = T)
      )) %>%
    ...
    for (i in 1:length(partidos)) {
      datos <- datos %>%
        mutate(conservar = (
          grepl(glue::glue("(\\b|_){partidos[i]}\\b"), partido1) &
            grepl(glue::glue("(\\b|_){partidos[i]}\\b"), partido2) |
            conservar
        ))
    ...
    datos %>%
      ...
      geom_text(aes(
        ...
        y = 0,
        color = partido1,
        label = glue::glue("{round(value,2)}")
      ), size = 10) +
      geom_text(aes(
        x = 1.5,
        y = 0,
        color = partido1,
        label = glue::glue("{partido1}"),
        vjust = 1,
        size = 10
      )) +
      coord_polar(theta = "y") +
      ...
      theme_void() +
      theme(
        text = element_text(color = "grey35", size = 20),
...
graficando_tabla <- function(bd,
  ...
  gt_tbl <- gt(data = final %>% select(-obs)) %>%
    fmt_percent(columns = vars(Porcentaje),
                decimals = 2)
